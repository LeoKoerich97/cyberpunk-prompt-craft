...
const HolographicCard = ({ children, className = '' }: HolographicCardProps) => {
  return (
    <div className={`relative group ${className}`}>
      ...
      <div className="relative bg-cyber-card border border-gray-700 rounded-lg p-8 hover:border-transparent transition-all duration-300 hover:shadow-holographic transform hover:-translate-y-1 hover:scale-105 will-change-transform">
        {children}
      </div>
    </div>
  );
